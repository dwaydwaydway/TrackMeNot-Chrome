...
TRACKMENOT.TMNSearch = function () {
    ...
    function startTMN() {
        ...
        //     { urls: ["https://www.google.com/complete/search?q*"] }
        // );
    ...
    function autoSuggestion(query) {
        debugger
        const url = "https://www.google.com/search?hl=en&q=|";
        const queryURL = queryToURL(url, query);
        var xhr = new XMLHttpRequest();
        xhr.open("GET", queryURL, true);
        xhr.send(null);
        xhr.onreadystatechange = function () {
    ...
    function autoSuggestionlistener(details) {
        chrome.webRequest.onBeforeRequest.removeListener(autoSuggestionlistener);
        debugger
