...
TRACKMENOT.TMNInjected = function() {
            ...
            if (testad(engine.id, anchorClass, anchorLink)) {
                ...
                if (j === clickIndex) {
                    ...
                    try {
                        ...
                        add_log({
                            'type': 'ERROR',
                            'query': "error opening click-through request for: " + e.message,
                            'engine': engine, 
                        });
        ...
        if (host === "" || !host.match(reg)) {
            ...
            if (searchBox && searchButton && engine !== 'aol') {
                ...
                try {
                    ...
                    add_log({
                        'type': 'ERROR',
                        'query': "Caught exception: " + ex.message,
                        'engine': engine, 
                    });
                    api.runtime.sendMessage({
                        "url": encodedUrl
    ...
    function add_log(msg) {
        msg.query = "[tmn_search.js] " + msg.query;
        ...
        handleRequest: function(request, sender, sendResponse) {
            ...
            if (request.click_eng) {
                try {
                    ...
                    add_log({
                        'type': 'ERROR',
                        'query': "Failed so click on results: " + ex.message,
                        'engine': engine, 
                    });
